...
describe('Insert element in asc order', function(){
  var sampleArray;
  beforeEach(function(){
    sampleArray = [7,5,3,1,4,6];
  });

  it('inserts element in order when 0 < index < length - 1', function(){
    expect(insert(sampleArray, 2, 1).equals([1,7,5,3,4,6])).to.equal(true);
  });

  it('inserts element in order when index of second element', function(){
